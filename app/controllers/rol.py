...
def create():
    try:
        ...
        return jsonify({
            "message": "Rol creado exitosamente",  # Mensaje adicional
            "rol": response  # Los datos del rol creado
        }), HTTPStatus.CREATED  # Código de estado 201
...
@rol_bp.route('/<int:id>/get',methods=["GET"])
def get_rol(id):
    try:
        rol = Rol.query.get(id)
        if not rol:
            raise GenericError(HTTPStatus.NOT_FOUND,HTTPStatus.NOT_FOUND.phrase,"Error Rol no encontrado..")
        return jsonify({
            "rol":RolWithPermissionSchema().dump(rol)
        })
    except GenericError:
        db.session.rollback()
        raise  # Relanza GenericError para que lo maneje @app.errorhandler
...
#lista 
@rol_bp.route('/list',methods=["GET","OPTIONS"])
def getAll():
    lista = Rol.get_active()
    #paginacion nativa de flask
    return jsonify(
            RolSchema().dump(lista,many=True)
        )
    
@rol_bp.route('/list-paginate',methods=["GET","OPTIONS"])
...
"""
@rol_bp.route('/<int:id_rol>/permiso/<int:id_permiso>/asignar',methods=["POST"])
def assigPermissionToRole(id_rol,id_permiso):
    try:
        rol = Rol.query.options(joinedload(Rol.permisos)).get(id_rol)
        ...
        db.session.commit()
        #return jsonify(RolSchema().dump(rol)) #respuesta anterior
        return jsonify({
            "message":"permiso Asignado con exito"
        })
...
@rol_bp.route("/<int:id_rol>/permiso/<int:id_permiso>/eliminar",methods=["DELETE"])
def deletePermissionForRole(id_rol,id_permiso):
    try:
        ...
        return jsonify({"message":"asignacion eliminada con exito"})
