...
def crearPermisos():
    ...
    #si encuentra el mismo permiso capaz y ya fue insertado entonces solamente cambia el estado de eliminado a false
        ...
        return jsonify({
            "permiso":PermisoSchema().dump(nuevo_permiso),
            "message":"Permiso creado con exito"}),HTTPStatus.CREATED
...
def update(id):
    try:
        ...
        return jsonify({
            "permiso":PermisoSchema().dump(permiso),
            "message":"Permiso actualizado con exito"}),HTTPStatus.CREATED
...
def getAll():
    roles = Permiso.get_active()
    return jsonify(PermisoSchema(many=True).dump(roles))


@permiso_bp.route('/list-paginate',methods=["GET","OPTIONS"])
def get_paginate_permissions():
    #para obtener todos al parecer el Permiso.query
    return PaginatedResponseT.paginate(Permiso.get_active(),PermisoSchema)
